const express = require("express");
const multer = require("multer");
const sharp = require("sharp");

// Create an instance of the Express Router to handle routes separately
const router = express.Router();

// Configure Multer to handle file uploads in memory (as a Buffer)
// memoryStorage() keeps the uploaded files in memory rather than saving them to disk
const storage = multer.memoryStorage();

// Initialize Multer with the configured storage setting
const upload = multer({ storage });

// Define a POST route to handle image resizing
// The `upload.single("image")` middleware is used to handle single image file uploads
router.post("/resize", upload.single("image"), async (req, res) => {
  // Check if an image file was uploaded
  if (!req.file) {
    // If no file is uploaded, respond with a 400 Bad Request and an error message
    return res.status(400).json({ error: "No file uploaded!" });
  }

  // Parse the width and height parameters from the request body and convert them to integers
  const width = parseInt(req.body.width, 10);
  const height = parseInt(req.body.height, 10);

  // Validate the width and height values, ensuring they are positive integers
  if (isNaN(width) || isNaN(height) || width <= 0 || height <= 0) {
    // If the width or height values are invalid, respond with a 400 Bad Request and an error message
    return res.status(400).json({ error: "Invalid width or height values!" });
  }

  try {
    // Determine the original file format by splitting the MIME type (e.g., "image/jpeg" -> "jpeg")
    const format = req.file.mimetype.split("/")[1];

    // Use the sharp library to resize the image to the specified width and height
    const resizedBuffer = await sharp(req.file.buffer)
      .resize(width, height) // Resize the image
      .toBuffer(); // Convert the image to a Buffer

    // Convert the resized image buffer to a base64-encoded string for easy transfer
    const resizedImage = resizedBuffer.toString("base64");

    // Send the resized image and its format back to the client as a JSON response
    res.json({ width, height, base64: resizedImage, format });
  } catch (error) {
    // If any error occurs during the image processing, log the error and respond with a 500 Internal Server Error
    console.error("Error during image processing:", error);
    res.status(500).json({ error: "Image resizing failed!" });
  }
});

// Export the router so it can be used in other parts of the application
module.exports = router;
